(ns edi_x12.main
  "Kotodama WASM entrypoint for the Edi_x12 clean-room actor (L4) — Clojure port.
  ...
  facts (`edi_x12.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "edi_x12")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem"   :plural "tradeitems"   :id-prefix "edix12_tra"
    :fields [:gtin :description :brand]          :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"       :plural "events"       :id-prefix "edix12_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {:itemId "TradeItem"}}
   {:entity "Document"    :plural "documents"    :id-prefix "edix12_doc"
    :fields [:type :senderId :receiverId :status] :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container"   :plural "containers"   :id-prefix "edix12_con"
    :fields [:bicCode :type :status]             :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"       :plural "parties"      :id-prefix "edix12_par"
    :fields [:gln :name :role]                   :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"    :plural "shipments"    :id-prefix "edix12_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"edi_x12.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "edi_x12-compat" :tier tier :entities entities} 200])
...
(defn start! [] :edi_x12-compat/ready)
